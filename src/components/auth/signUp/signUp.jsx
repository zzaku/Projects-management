...
export const SignUp = ({ setRegister, pathLocation, setNeedToConnect, mobile }) => {
  const [user, setUser] = useState({});
  const [inscription, setInscription] = useState({});
  const [error, setError] = useState("");
  const { signup, addInfosUser } = useFirebase();
  ...
  async function submitInscription(e) {
    ...
    if (user.password && (user.password.length < 8 || !/[A-Z]/.test(user.password) || !/[a-z]/.test(user.password) || !/[!@#$%^&*]/.test(user.password))) {
  ...
  return (
    <div className="signin-container d-flex content-center item-center">
      <form className="form">
          ...
          {inscription.successInscription && <MuiAlert severity="success">{inscription.successInscription}</MuiAlert>}
          <div className="field">
          ...
          <div className="btn">
          <Button variant="outained">S'inscrire</Button>
